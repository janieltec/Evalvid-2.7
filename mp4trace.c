...
static int enqueue_v(void *context)
{
  ...
  if (ctx->mode & MODE_FRAME && ctx->rtp->M) {
    ...
    send_time  = (double)time_now.tv_sec + (double)time_now.tv_usec/1000000.0;
//    printf("%u\t%c\t%u\t%u\t%.3f\n", ++ctx->count, ctx->frametype["HIPBXXX"], ctx->framesize, ctx->segm, send_time);
